...
impl LeveledCompactionController {
    ...
    fn find_overlapping_range(
        ...
        (
            ...
            end_index.unwrap_or(snapshot.levels[in_level - 1].1.len()),
        )
    }
    ...
    fn compute_target_level_size(&self, snapshot: &LsmStorageState) -> Vec<(usize, usize)> {
        ...
        target_sizes.push((
            ...
            max(
                ...
                snapshot.levels[self.options.max_levels - 1]
                    .1
                    .iter()
        ...
        for (level, _) in snapshot.levels.iter().rev().skip(1) {
            if target_sizes[0].1 <= self.options.base_level_size_mb * 1024 * 1024 {
                ...
                target_sizes.insert(
                    ...
                    (
                        *level,
                        target_sizes[0].1 / self.options.level_size_multiplier,
                    ),
                );
            }
        }
    ...
    pub fn generate_compaction_task(
        ...
        if snapshot.l0_sstables.len() >= self.options.level0_file_num_compaction_trigger {
            let target = target_sizes
                .iter()
                .find(|target_size| target_size.1 != 0)
                .unwrap();
            let overlap_range =
                self.find_overlapping_range(snapshot, &snapshot.l0_sstables, target.0);
            return Some(LeveledCompactionTask {
                upper_level: None,
                upper_level_sst_ids: snapshot.l0_sstables.clone(),
                lower_level: target.0,
                lower_level_sst_ids: snapshot.levels[target.0 - 1].1
                    [overlap_range.0..overlap_range.1]
                    .to_vec(),
        ...
        if result.1 > 1.0 {
            ...
            let overlap_range =
                self.find_overlapping_range(snapshot, &[oldest_sst_id], result.0 + 1);
            return Some(LeveledCompactionTask {
                upper_level: Some(result.0),
                upper_level_sst_ids: vec![oldest_sst_id],
                lower_level: result.0 + 1,
                lower_level_sst_ids: snapshot.levels[result.0].1[overlap_range.0..overlap_range.1]
                    .to_vec(),
    ...
    pub fn apply_compaction_result(
        ...
        if task.upper_level.is_none() {
            ...
            state.levels[task.upper_level.unwrap() - 1]
                .1
