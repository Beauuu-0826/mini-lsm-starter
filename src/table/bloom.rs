...
impl Bloom {
    ...
    pub fn decode(buf: &[u8]) -> Result<Self> {
        let bytes = &buf[..buf.len() - 4];
        if crc32fast::hash(bytes) != (&buf[buf.len() - 4..]).get_u32() {
            bail!("The bloom filter has corrupted");
        }

        let filter = &bytes[..bytes.len() - 1];
        let k = bytes[bytes.len() - 1];
        Ok(Self {
